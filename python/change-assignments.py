...
'''
run this as `python change-assignments.py someone@example.com user`
to change to user
`python change-assignments.py someone@example.com group`
to change to group
'''

domain = 'example'
base_apps_url = f"https://{domain}.okta.com/api/v1/apps"
base_users_url = f"https://{domain}.okta.com/api/v1/users"

# this relies on okta_token variable in env file
# if you don't have that, sub out the token here
...
# get the user ID from the email by searching the email
# and returning the okta ID
...
def main():
    ...
    # this could probably be changed to an ELIF
    # for user and error if anything but group or user
